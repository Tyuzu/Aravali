...
export async function updateBanner(): Promise<boolean> {
  ...
  try {
    ...
    Notify("Banner updated successfully.", { type: "success", duration: 3000, dismissible: true });

    const preview = document.getElementById("banner-picture-preview") as HTMLImageElement | null;
    if (preview) {
      const basePath = resolveImagePath(EntityType.USER, PictureType.BANNER, response.banner);

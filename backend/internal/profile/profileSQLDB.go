...
func SQLFindUserByFilter(ctx context.Context, app *infra.Deps, filter map[string]any) (*auth.User, error) {
	return findUser(ctx, filter, app.DB)
}

func SQLApplyProfileUpdatesDeps(ctx context.Context, app *infra.Deps, userID string, updates map[string]any) (any, error) {
...
func SQLfindUser(ctx context.Context, filter map[string]any, database db.Database) (*auth.User, error) {
	var user auth.User
	_ = database.FindOne(ctx, usersTable, filter, &user)
...
func SQLRespondWithUserProfile(w http.ResponseWriter, userid string, database db.Database) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	var userProfile auth.User
	_ = database.FindOne(ctx, usersTable, map[string]any{"userid": userid}, &userProfile)
